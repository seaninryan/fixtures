...
export function announceLines(fixtures, config, windowName, today) {
  ...
  // ONE chronological list, both sources interleaved. The reader is a club member
  // pasting this into a WhatsApp group: which back-end system a fixture came from is
  // plumbing, and a Saturday reads better as a Saturday than as two of them.
  //
  // `labels` above came from resolveTeams over the whole union, and that is load-bearing
  // however these lines are grouped: deriveLabels decides whether to show the A/B letter
  // by COUNTING the club's squads at that age and gender in the list it is given, so
  // announcing each source separately would silently rename "U14A Boys" to "U14 Boys".
  // That bug has appeared three times here. Keep this ONE announceLines call over the
  // union - never one call per source.
  let currentDay = null;
